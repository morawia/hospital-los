...
The statistical analysis shows that the target variable slos has a mean length of stay of approximately 18 days, with a median of 11 days and a very strong right skew (skewness ≈ 4.6).
...
We created Pearson and Spearman correlation heatmaps and bar plots (predictor–LOS), histograms for numeric predictors and LOS, frequency plots for categorical variables, and boxplots of LOS by categorical predictors (both uncapped and capped at the 99th percentile for more readability).
...
Sex is moderately imbalanced (≈56\% male, 44\% female), while DNR status shows strong imbalance, with the majority of patients having no DNR order and much smaller groups with DNR before or after admission. 
